...
// Simpan hash dan plain password untuk keperluan admin
$stmt = $pdo->prepare("INSERT INTO users (nama, email, kata_sandi, plain_password, no_telepon, avatar_warna) VALUES (?, ?, ?, ?, ?, ?)");
try {
    $stmt->execute([$nama, $email, $hash, $kataSandi, $noTelepon, $warna]);
} catch (Exception $e) {
    // Fallback jika kolom plain_password belum ada
    $stmt2 = $pdo->prepare("INSERT INTO users (nama, email, kata_sandi, no_telepon, avatar_warna) VALUES (?, ?, ?, ?, ?)");
    $stmt2->execute([$nama, $email, $hash, $noTelepon, $warna]);
}
